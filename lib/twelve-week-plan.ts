...
import { resourcesForDomain } from "@/lib/learning-resources";
...
type DomainPlan = { focusEn: string; focusUz: string; actionsEn: string[]; actionsUz: string[]; resourceOrder: [number, number, number, number, number]; resultEn: string; resultUz: string };
  ...
  return sequence.map((domain, index) => {
    ...
    return {
      ...
      tasks: ([1, 2, 3, 4, 5, 6] as const).map((day) => {
        ...
        const resource = resources[plan.resourceOrder[day - 1]];
        return {
          id: `w${week}-d${day}`,
          day,
