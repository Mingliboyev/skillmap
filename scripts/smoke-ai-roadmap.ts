...
const { v2ActiveCoreItems } = await import("../lib/assessment/v2-bank");
const { scoreV2 } = await import("../lib/assessment/v2-results");
const { generateV2Roadmap } = await import("../lib/ai/v2-roadmap");
const answers = Object.fromEntries(v2ActiveCoreItems.map((item, index) => [item.itemId, index < 7 ? (item.correctOption === "A" ? "B" : "A") : item.correctOption]));
const evidence = scoreV2(v2ActiveCoreItems.map((item) => item.itemId), answers, "uz", "grade_10_11", "software-engineering");
