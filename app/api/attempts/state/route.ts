...
import { V2_BANK_VERSION, V2_SUPPORTED_VERSIONS } from "@/lib/assessment/v2-bank";
...
export async function GET(){const ctx=await context();if(!ctx)return NextResponse.json({status:"none",authenticated:false});const{data}=await ctx.supabase.from("assessment_attempts").select("id,status,started_at,completed_at,overall_score,progress,result_summary").eq("student_id",ctx.user.id).in("assessment_version",[...V2_SUPPORTED_VERSIONS]).order("started_at",{ascending:false}).limit(1).maybeSingle();return NextResponse.json(data?{authenticated:true,status:data.status,attemptId:data.id,startedAt:data.started_at,completedAt:data.completed_at,overallScore:data.overall_score,progress:data.progress,result:data.result_summary}:{authenticated:true,status:"none"})}
