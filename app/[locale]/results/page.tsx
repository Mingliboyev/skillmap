...
export default async function Results({ params }: { params: Promise<{ locale: string }> }) {
  ...
  let initial: null | { result: V2EvidenceSummary; roadmap: V2Roadmap } = null;
  if (supabase) {
    const { data: { user } } = await supabase.auth.getUser();
    if (user) {
      const { data } = await supabase.from("assessment_attempts")
        .select("result_summary,roadmaps(plan)")
        ...
        .limit(1)
        .maybeSingle();
      const roadmap = (data?.roadmaps as unknown as { plan: V2Roadmap }[] | null)?.[0]?.plan;
      if (data?.result_summary && roadmap) initial = { result: data.result_summary as V2EvidenceSummary, roadmap };
    }
  }
  return <ResultsReport locale={locale} initial={initial}/>;
}
