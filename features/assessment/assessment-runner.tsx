...
export function AssessmentRunner({ locale, attemptId, initialProgress }: { locale: Locale; attemptId?: string; initialProgress?: StoredProgress }) {
  ...
  return <>{submitting&&<div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/65 p-5 backdrop-blur-sm" role="status" aria-live="polite"><section className="w-full max-w-md rounded-3xl bg-white p-7 text-center shadow-2xl sm:p-9"><span className="mx-auto grid size-16 place-items-center rounded-2xl bg-teal-100 text-teal-800"><LoaderCircle className="animate-spin" size={32}/></span><p className="mt-6 flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-widest text-teal-700"><Sparkles size={15}/>{uz?"AI ishlayapti":"AI is working"}</p><h2 className="mt-3 text-2xl font-bold">{uz?"Natijangiz tayyorlanmoqda":"Preparing your results"}</h2><p className="mt-3 leading-7 text-slate-600">{uz?"Javoblaringiz tahlil qilinmoqda va siz uchun shaxsiy 12 haftalik roadmap tayyorlanmoqda. Bu odatda bir necha soniya davom etadi.":"Your answers are being analyzed and your personal 12-week roadmap is being prepared. This usually takes a few seconds."}</p><div className="mt-6 h-2 overflow-hidden rounded-full bg-slate-200"><span className="block h-full w-2/3 animate-pulse rounded-full bg-teal-700"/></div><p className="mt-3 text-xs font-semibold text-slate-500">{uz?"Iltimos, sahifani yopmang.":"Please keep this page open."}</p></section></div>}<div className="container-shell py-8 lg:py-14"><div className="mx-auto max-w-3xl">
