...
export function ResultsReport({ locale, initial }: { locale: Locale; initial?: { result: V2EvidenceSummary; roadmap: V2Roadmap } | null }) {
  ...
  return <div className="container-shell py-10 lg:py-16"><div className="mx-auto max-w-6xl">
    <header className="flex flex-wrap items-end justify-between gap-5"><div><Badge>{uz ? "30 savollik diagnostik hisobot" : "30-question diagnostic report"}</Badge><h1 className="display mt-4 text-4xl">{uz ? "Ko‘nikmalar xaritangiz" : "Your skill map"}</h1></div><div className="no-print flex flex-wrap gap-2"><Button onClick={() => print()}><Printer size={17}/>{uz ? "Chop etish" : "Print"}</Button><Link className={buttonClass("primary")} href={`/${locale}/roadmap#twelve-week-plan`}><CalendarDays size={17}/>{uz ? "12 haftalik rejani ochish" : "Open 12-week plan"}</Link></div></header>
